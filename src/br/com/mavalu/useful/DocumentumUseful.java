...
public class DocumentumUseful {
    ...
    * @param type Define o tipo de query. 0 para select e 1 para update. Existem outros tipos que podem ser
    * implementados.
      ...
      try {
       ...
       /**  int length = doc.getValueCount("anomalia");
         ...
         for (int i = 0; i < length; ++i) {
            anomalia += doc.getRepeatingString("anomalia", i);
            if ((i + 1) < length) {
               anomalia += "|";
            }
         }
   ...
   private static class FirstSession
      ...
      public void run() {
         ...
         try {
            ...
            DtoolLogControl.log("Sessão Liberada", Level.INFO);
